...
public enum AladinApi {
    ...
    enum List {
        ...
        /// 신간 전체 리스트
        static let ItemNewAll = "QueryType=ItemNewAll"
        /// 주목할 만한 신간 리스트
        static let ItemNewSpecial = "QueryType=ItemNewSpecial"
        /// 편집자 추천 리스트
        static let ItemEditorChoice = "QueryType=ItemEditorChoice"
        /// 베스트셀러
        static let Bestseller = "QueryType=Bestseller"
        /// 블로거 베스트셀러 (국내도서만 조회 가능)
        static let BlogBest = "QueryType=BlogBest"
    }
    /// apiKey
    static let apiKey = "ttbjoony30001924001"
    /// 표시할 페이지
    /// - Parameter num: 1000 / resultPerPage 만큼 가능
    static func Page(num: Int) -> String { return "start=\(num)" }
    /// 검색결과 한 페이지당 최대 출력 개수 총 1000개 결과값
    /// - Parameter num: 1~50
    static func resultPerPage(num: Int) -> String { return "maxResults=\(num)" }
    /// 출력 형식
    /// - Parameter form: xml or json
    /// - Returns: "output=형식"
    static func outPut(form: Form) -> String {
        switch form {
